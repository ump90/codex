...
def _git_for_windows_root_from_bash(bash_path: Path) -> Optional[Path]:
    ...
    if parent.name.lower() != "bin":
        return None
    root = (
        parent.parent.parent if parent.parent.name.lower() == "usr" else parent.parent
    )
    if (root / "cmd" / "git.exe").exists() and (
...
def main() -> int:
    ...
    if git_bash:
        ...
        add(
            ...
            rc == 0
            and assert_exists(target)
            and target.read_text(encoding="utf-8") == "ok",
            f"rc={rc}, err={err}",
        )
        ...
        add(
            "RO: Git Bash install root readable",
            rc == 0,
            f"rc={rc}, git_root={git_root}, err={err}",
        )
    else:
